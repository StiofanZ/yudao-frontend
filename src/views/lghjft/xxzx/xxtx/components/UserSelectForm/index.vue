...
const open = async (id: number, selectedList?: any[], deptIds?: number[]) => {
  ...
  if (limitDeptIds.value.length > 0) {
    ...
    limitDeptIds.value.forEach(id => {
      const childIds = getChildDeptIds(id, deptData)
      childIds.forEach(childId => validDeptIds.add(childId))
    })
    
    // 过滤部门数据
    const filteredDeptData = deptData.filter((item: any) => validDeptIds.has(item.id))
    deptTree.value = handleTree(filteredDeptData)
  } else {
