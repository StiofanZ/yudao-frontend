...
defineOptions({ name: 'XxzxTzggForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await TzggApi.getTzgg(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as TzggApi.TzggVO
    if (formType.value === 'create') {
      await TzggApi.createTzgg(data)
      message.success(t('common.createSuccess'))
    } else {
      await TzggApi.updateTzgg(data)
